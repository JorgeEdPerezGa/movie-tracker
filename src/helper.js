...
export const cleanMovies = (response) => {
  const movies = response.results.map(({ title, overview, poster_path, backdrop_path, id, vote_average }) => {
    return { title, overview, poster_path, backdrop_path, id, vote_average };
  });
...
export const addFavorite = async user => {
  try {
    const url='/api/users/new';
    const register = await fetch(url, {
      method: "POST",
